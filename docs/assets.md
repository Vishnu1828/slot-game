...
- `{nomip}` — AssetPack's `mipmap` pipe generates no resolution variants. **This does not mean the folder
  has no tiers.** Pre-baked sprite-sheet folders keep `{nomip}` because `mipmap` would resize whole sheets
  and never touch the sibling `.json`; their `@0.5x` / `@0.25x` copies are generated instead by
  `assetpack/prebakedSheetTiers.mjs`, which rewrites both halves. Which sheets tier is decided by location
  (everything in a `<game>-win` bundle) or by a top-level `"tier": true | false` in the sheet's own JSON —
  **never by filename**, because sheet names are per-game data. See `docs/animations.md`.
- `{nc}` — do not compress.
...
Note the loads **do** overlap even though the beats don't: the popup has to be fetched *during* the glow beat
to be ready in time. That is why `releaseSheets` never unloads a sheet mid-load — see below.

Demand-loading alone was not enough, because the cheapest thing it could load was still 160 MB. These sheets
also get **resolution tiers** (`@0.5x` / `@0.25x`) generated by `assetpack/prebakedSheetTiers.mjs`, and
everything in a `<game>-win` bundle tiers by default. On a phone (tier 0.5) idle texture is ~80 MB and the
peak during a win ~130 MB, against ~205 MB and ~417 MB at full resolution. See `docs/animations.md`.

Three rules in [`winAssets.ts`](../src/game/winAssets.ts) are load-bearing, each fixing a bug that shipped:

- **Load the `.json`, not the `.png`.** A TexturePacker sheet fetches its own atlas via `meta.image`, so
  asking for `<base>.png` as well decodes the same pixels again under a different URL *and* format — a second
  copy nothing reads, doubling all win art. Only the custom `sprites[]` dialect needs its atlas by alias.
- **Load and free of one sheet take turns** (one promise chain per sheet). `Assets.unload` awaits an
  in-flight load then destroys what it produced, while the load's own continuation has already run
  `Cache.set` — leaving the alias **cached but destroyed**: renders nothing, never self-repairs. Do not fix
  that by *skipping* a colliding free; that drops it with nothing to retry and art accumulates every spin.
- **`hasAsset` checks liveness** (`src/utils/assets.ts`), so a cached-but-destroyed sheet reports missing and
  the next win re-fetches it.
